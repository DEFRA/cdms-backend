...
public class Generator(ILogger<Generator> logger, IBlobService blobService)
{
    ...
    {
        logger.LogInformation("Uploading {ImportNotificationsLength} Notification(s) and {ClearanceRequestsLength} Clearance Request(s) to blob storage", result.ImportNotifications.Length, result.ClearanceRequests.Length);
        ...
        var success = await blobService.CreateBlobsAsync(importNotificationBlobItems.Concat(alvsClearanceRequestBlobItems).ToArray<IBlobItem>());
        return success;
    }
}