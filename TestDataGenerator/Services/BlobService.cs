...
public abstract class AzureService<T>
{
    ...
    protected AzureService(ILogger<AzureService<T>> logger, IAzureConfig config,
        IHttpClientFactory? clientFactory = null)
    {
        ...
        {
            logger.LogInformation("Creating azure credentials based on config vars for {ConfigAzureClientId}",
    ...
    : AzureService<BlobService>(logger, config, clientFactory), IBlobService
{
    ...
    {
        Logger.LogInformation("Cleaning blob storage {ConfigDmpBlobUri} : {ConfigDmpBlobContainer} / ${Prefix}",
            config.DmpBlobUri, config.DmpBlobContainer, prefix);
    ...
    public async Task<bool> CreateBlobsAsync(IBlobItem[] items)
    {
        Logger.LogInformation("Connecting to blob storage {DmpBlobUri} : {DmpBlobContainer}", config.DmpBlobUri,
