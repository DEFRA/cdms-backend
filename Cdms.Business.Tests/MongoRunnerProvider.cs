...
// Copied and pasted from other test project, need to refactor out to share
internal sealed class MongoRunnerProvider
{
    ...
    private sealed class MongoRunnerWrapper(MongoRunnerProvider owner, IMongoRunner underlyingMongoRunner)
        : IMongoRunner
    {
        private readonly MongoRunnerProvider _owner = owner;
        private IMongoRunner? _underlyingMongoRunner = underlyingMongoRunner;

        public string ConnectionString => _underlyingMongoRunner?.ConnectionString ??
                                          throw new ObjectDisposedException(nameof(IMongoRunner));

        public void Import(string database, string collection, string inputFilePath, string? additionalArguments = null,
            bool drop = false)
        {
        ...
        public void Export(string database, string collection, string outputFilePath,
